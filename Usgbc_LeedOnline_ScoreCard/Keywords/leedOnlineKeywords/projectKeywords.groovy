...
class projectKeywords {
		...
		if (dirContents.length > 0) {
			...
			for (int i = 0; i < dirContents.length; i++) {
				...
				if (dirContents[i].getName().equals(fileName)) {
					...
					println "File name from Excel "+dirContents[i].getName() + " equals to downloaded file name " + fileName
	...
	def compareNarrativeDataWithPDF(String scroll,String Editorpath) {
		...
		def Narrative = WebUI.getText(findTestObject(Editorpath))
		println "#############################"+ Narrative
	...
	def public callNavigationTestCase(String formName, String formv01){
		...
		WebUI.delay(3)
		WebUI.scrollToElement(findTestObject('Login/span_End User License Agreement'), 30)
		boolean userAgreemntVisible = WebUI.waitForElementVisible(findTestObject('Login/span_End User License Agreement'), 30)
		if (userAgreemntVisible == true){
			WebUI.check(findTestObject('Login/span_End User License Agreement'))
		}
		else{
		...
		TestObject projectName  = new TestObject().addProperty('xpath', ConditionType.EQUALS, '//div[@title="BDD TestProject 1"]', true)
		WebUI.click(projectName)
		WebUI.click(findTestObject('Login/a_Credits'))
		WebUI.delay(2)
	...
	def compareUploadedFileWithPDF1(String Table_Xpathid,String scrollTo,String iframe) {
		...
		WebElement Table = WebUiCommonHelper.findWebElement(findTestObject(Table_Xpathid), 10)
		...
		for (int row = 0; row < rows_count; row++) {
			...
			for (int column = 0; column <2; column++) {
				...
				def celltext = Columns_row.get(col1).getText()
				println(celltext)
				col1++
				if(file.contains(celltext)) {
					println("The content in the downloaded pdf matches with the Form Content --> Found")
	...
	@Keyword
	def public clickCheckbox(String xpath, String iframe){
		WebDriver driver = DriverFactory.getWebDriver()
		//println "################### xpath is "+xpath
		WebUI.switchToFrame(findTestObject(iframe), 10)

		//WebElement checkbox =driver.findElement(By.xpath(xpath))
		WebElement checkbox = WebUiCommonHelper.findWebElement(findTestObject(xpath), 10)
		if(!checkbox.isSelected()){
			println"Checked ##########"
			checkbox.click()
		}
	...
	// Repeated function with name change -- Ended


	@Keyword
	def countRowsPerTable(String xpath, String iframe){

		WebUI.switchToFrame(findTestObject(iframe), 5)
		WebElement Table = WebUiCommonHelper.findWebElement(findTestObject(xpath), 10)
		List < WebElement > totalNoOfRows = Table.findElements(By.tagName('tr'))
		//List
	...
	@Keyword
	def verifyUploadedFiles(String tableRow, String tableId, String iframe ){
		//println "The tableRow xpath is "  + tableRow
		println "The tableRow xpath is "  + tableRow
		println "The tableId xpath is "  + tableId
		String fileName
		int count = (new projectKeywords()).countRowsPerTable(tableRow,iframe)
		println "The table count is "+ count
	...
	@Keyword
	def downloadUploadedFiles(String tableRow, String tableId, String scrollTo, String iframe ){
	...
	@Keyword
	def deleteUploadedFiles(String tableRow, String tableId, String scrollTo, String iframe){

		int row_Count = (new projectKeywords()).countRowsPerTable(tableRow,iframe)
		println "Count in table : "+row_Count
	...
	@Keyword
	def changeTheUnits(String unitType, String formname, String spanV01){

		WebUI.back()
		WebUI.delay(2)
		WebUI.click(findTestObject('Object Repository/Common/UnitTypes/a_Details'))
		WebUI.delay(2)
		WebUI.click(findTestObject('Object Repository/Common/UnitTypes/Edit_Btn'))
		WebUI.delay(5)
		if(unitType=="SI"){
			WebUI.click(findTestObject('Object Repository/Common/UnitTypes/input_SI'))
		}
		else if(unitType=="IP"){
			WebUI.click(findTestObject('Common/UnitTypes/input_IP'))
		}
		else{
			println "Use SI or PI for Unit type"
		}
		WebUI.delay(2)
		WebUI.click(findTestObject('Object Repository/Common/UnitTypes/update_Btn'))
		WebUI.delay(5)
		WebUI.click(findTestObject('Object Repository/Common/UnitTypes/a_Credits'))
		WebUI.delay(3)
		WebUI.click(findTestObject(formname))
		WebUI.delay(3)
		WebUI.click(findTestObject(spanV01))
		WebUI.delay(3)
	}
